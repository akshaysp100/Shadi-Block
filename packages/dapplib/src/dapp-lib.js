...
const { serialize } = require('ethers/utils/transaction');
const borsh = require("borsh");
const lo = require("buffer-layout");
const { range } = require('lodash-es');
///+import

let DUMMY_TX_ID = "aaaaaaaaaaaaaaaa";
let DUMMY_CREATED_ON = "0000000000000000";
let DUMMY_DATE = "00-00-0000";
let DUMMY_PLACE = "aaaaaaaaaaaaaaaa";

class LoveRecord {
    constructor(fields = undefined) {
        this.groom = DUMMY_TX_ID;
        this.bride = DUMMY_CREATED_ON;
        this.date = DUMMY_DATE;
        this.place = DUMMY_PLACE;
        if (fields) {
...
module.exports = class DappLib {
    ...
    static async countHellos(data) {
        ...
        let groom = lo.cstr("groom");
        let bride = lo.cstr("bride");
        let date = lo.cstr("date");
        let place = lo.cstr("place");

        const ds1 = lo.struct([groom, lo.seq(lo.u8(), 2), bride, lo.seq(lo.u8(), 2), date, lo.seq(lo.u8(), 2), place, lo.seq(lo.u8(), 2)]);

        let res = [];
        for (let i = 0; i < 15; i++) {
            let ans = ds1.decode(result.callData[i]);
            let groom = ans.groom.slice(0, 16); groom = groom.split('.')[0];
            ...
            if (!(groom === "aaaaaaaaaaaaaaaa")) {
                let bride = ans.bride.slice(0, 16); bride = bride.split('.')[0];
                let date = ans.date.slice(0, 10);
                let place = ans.place.slice(0, 16); place = place.split('.')[0];
                let lov = new LoveRecord();
                lov.groom = groom;
                lov.bride = bride;
                lov.date = date;
                lov.place = place;
    ...
    static async sayHello(data) {
        ...
        loveobj.groom = data.groom.padEnd(16, '.');
        loveobj.bride = data.bride.padEnd(16, '.');
        loveobj.date = data.date;
        loveobj.place = data.place.padEnd(16, '.');

        let res = borsh.serialize(LoveRecordSchema, loveobj);
